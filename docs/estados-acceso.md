...
`CARD_UNKNOWN`, `CARD_BLOCKED`, `CARD_LOST`, `FACE_MISMATCH`, `FACE_PROVIDER_ERROR`, `PERMISSION_DENIED`, `PERMISSION_EXPIRED`, `ALREADY_INSIDE`, `ALREADY_OUTSIDE`, `CAPACITY_FULL`, `ACTIVE_REQUEST_EXISTS`, `DEVICE_UNKNOWN`, `REQUEST_EXPIRED`, `CONFIG_EXPIRED`, `EDGE_UNAVAILABLE`, `SYNC_CONFLICT`, `OFFLINE_VISITOR_REQUIRES_GUARD`, `LOCKDOWN_ACTIVE`, `UNAUTHORIZED` y `FORBIDDEN`.
